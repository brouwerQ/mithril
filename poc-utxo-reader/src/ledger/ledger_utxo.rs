...
use super::interface::Ledger;

pub type TransactionsHistory = Vec<TransactionAddressRecord>;
pub type AddressesTransactionsHistory = BTreeMap<Address, TransactionsHistory>;
pub type UnspentTransactionxOutputs = HashMap<TransactionOutputRef, TransactionOutput>;

#[derive(Debug, Default)]
pub struct LedgerWithUTxO {
    address_transactions: AddressesTransactionsHistory,
    utxos: UnspentTransactionxOutputs,
}

impl Ledger<TransactionsHistory, AddressesTransactionsHistory> for LedgerWithUTxO {
    /// Save a transaction to the ledger
    fn save_transaction(&mut self, tx: &Transaction) -> StdResult<()> {
    ...
    fn records_for_address(&self, address: &Address) -> Option<TransactionsHistory> {
        self.get_transactions_for_address(address)
            .map(|v| v.to_vec())
    }

    fn records_for_all_addresses(&self) -> &AddressesTransactionsHistory {
...
mod tests {
    ...
    fn ledger_should_save_and_delete_utxos() {
        let mut ledger = LedgerWithUTxO::default();
